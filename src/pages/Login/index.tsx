...
export function Login() {
  ...
  return (
    <div className="min-h-screen bg-bg-secondary flex items-center justify-center px-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md bg-white rounded-3xl shadow-soft border border-slate-100 p-8 space-y-6">
        ...
        <div>
          ...
          <input value={password} onChange={(e) => setPassword(e.target.value)} type="password" required className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-accent-pink/20" />
        </div>
        ...
        <p className="text-center text-sm text-slate-500">
          New to Mnemo AI? <Link to="/signup" className="text-accent-pink font-medium">Create an account</Link>
        </p>
      </form>
    </div>
  );
